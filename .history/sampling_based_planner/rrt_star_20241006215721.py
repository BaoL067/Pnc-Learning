...
class RRTStar:
    ...
    def backtracking(self, goal_idx):
        ...
        """
        [node for node in self.node_list if math.hypot(node.x - new_node.x, node.y - new_node.y) <= self.near_nodes_dist_threshold]
        target_nodes = min(self.node_list, key=lambda node: math.hypot(node.x - self.end.x, node.y - self.end.y))
