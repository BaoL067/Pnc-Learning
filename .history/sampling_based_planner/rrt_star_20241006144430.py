...
class RRTStar:
    ...
    def generate_path(self, node):
        ...
        node.path_x = np.linspace(node.parent.x, node.x, steps).tolist()
        node.path_y = np.linspace(node.parent.y, node.y, steps).tolist()

        if not check_collision_with_obs_map(node, self.obstacle_map):
            node.reset()
            return False
        return True
    ...
    def rewire(self, node, new_parent):
        ...
        if self.generate_path(node):
            node.cost = new_parent.cost + math.hypot(node.x - new_parent.x, self.y - new_parent.y)
