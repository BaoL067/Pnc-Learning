...
img_path = str(pathlib.Path.cwd()) + r"\sampling_based_planner\rrt_star\maps\map3.png"
...
class RRTStar:
    ...
    def planning(self):
        ...
        for i in range(self.max_iter):
            ...
            dx, dy = random_node.x - nearest_node.x, random_node.y - nearest_node.y
            d = math.hypot(dx, dy)
            ...
            if ((not self.search_until_max_iter) and new_node):
                if (math.hypot(new_node.x - self.end.x, new_node.y - self.end.y) <= self.extend_dist):
                    ...
                    if not self.check_collision_with_obs_map(self.end, self.obstacle_map):
                        self.end.reset()
                        continue
    ...
    def backtracking(self, goal_idx):
        ...
        """
        target_nodes = [node for node in self.node_list if math.hypot(node.x - self.end.x, node.y - self.end.y) <= self.extend_dist]
        if (target_nodes == []):
            print("Cannot find path")
            return None
        node = min(target_nodes, key=lambda node: node.cost + math.hypot(node.x - self.end.x, node.y - self.end.y))
        # min(self.node_list, key=lambda node: math.hypot(node.x - self.end.x, node.y - self.end.y))
    ...
    def check_collision_with_obs_map(self, node, obstacle_map):
        if node is None or not (0 <= round(node.x) < len(obstacle_map) and 0 <= round(node.y) < len(obstacle_map[0])):
            return False

        if obstacle_map[round(node.x)][round(node.y)]:
            return False
