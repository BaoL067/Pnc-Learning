            ...
            if self.check_collision_with_obs_map(new_node, self.obstacle_map):
                ...
                for near_node in near_nodes:
                    if near_node.cost > new_node.cost + math.hypot(near_node.x - new_node.x, near_node.y - new_node.y):
                        ...
                        if self.check_collision_with_obs_mapcheck_collision_with_obs_map(near_node, self.obstacle_map):
                            near_node.parent = new_node
                            near_node.generate_path()
                            near_node.cost = new_node.cost + math.hypot(near_node.x - new_node.x, near_node.y - new_node.y)
                        self.draw_node_and_edge(new_node)
            else:
                new_node.reset()
                continue
