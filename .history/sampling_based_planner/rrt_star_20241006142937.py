...
class RRTStar:
    class Node:
        ...
        def rewire(self, new_parent):
            ...
            self.generate_path()
            self.cost = new_parent.cost + math.hypot(self.x - new_parent.x, self.y - new_parent.y)
        ...
        def generate_path(self):
            ...
            steps = int(round(d / RRTStar.get_path_resolution(RRTStar)))

            self.path_x = np.linspace(self.parent.x, self.x, steps).tolist()
            self.path_y = np.linspace(self.parent.y, self.y, steps).tolist()
            RRTStar.check_collision_with_obs_map(RRTStar, self, RRTStar.get_obs_map())
    ...
    def planning(self):
        ...
        for i in range(self.max_iter):
            ...
            if self.check_collision_with_obs_map(new_node, self.obstacle_map):
                ...
                optimal_node = min(near_nodes, key=lambda node: node.cost + math.hypot(node.x - new_node.x, node.y - new_node.y))
                new_node.rewire(optimal_node)
                # while near_nodes:
                ...
                for near_node in near_nodes:
                    if near_node.cost > new_node.cost + math.hypot(near_node.x - new_node.x, near_node.y - new_node.y):
                        near_node.rewire(new_node)
                        if not self.check_collision_with_obs_map(near_node, self.obstacle_map):
                            near_node.reset()
                            continue
    ...
    def get_path_resolution(self):
        return self.path_resolution
