            ...
            if self.check_collision_with_obs_map(new_node, self.obstacle_map):
                ...
                if self.check_collision_with_obs_map(new_node, self.obstacle_map):
                    for node in near_nodes:
                        if node.cost > new_node.cost + math.hypot(node.x - new_node.x, node.y - new_node.y):
                            node.parent = new_node
                            node.generate_path(new_node)
                            node.cost = new_node.cost + math.hypot(node.x - new_node.x, node.y - new_node.y)
