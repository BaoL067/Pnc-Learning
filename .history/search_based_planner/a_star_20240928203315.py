    ...
    def search(self, start_x, start_y, goal_x, goal_y):
        ...
        while open_set:
            ...
            for i, (dx, dy, cost) in enumerate(self.motion_model):
                ...
                # check if the next node is already in the closed set
                if self.get_vec_index(next_node) in close_set:
                    continue
                ...
                if next_node not in open_set:
                    ...
                    if next_node.cost > next_cost:
                        next_node.cost = next_cost
                        next_node.parent_idx = self.get_vec_index(current_node)
                        heapq.heappush(open_set, (next_node.cost, next_node))
