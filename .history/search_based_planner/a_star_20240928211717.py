...
class AStarPlanner:
    ...
    def search(self, start_x, start_y, goal_x, goal_y):
        ...
        while open_set:
            ...
            if self.get_vec_index(current_node)  in close_set:
                continue

            # add the current node to the closed set
            close_set[self.get_vec_index(current_node)] = current_node
            nodes_expanded += 1
            ...
            for i, (dx, dy, cost) in enumerate(self.motion_model):
                ...
                if not self.check_node_validity(next_node) or self.get_vec_index(next_node) in close_set:
                    continue
                
                next_cost = current_node.cost + cost
                if self.get_vec_index(next_node) not in close_set or next_cost < close_set[self.get_vec_index(next_node)].cost:
                    next_node.cost = next_cost
                    next_node.cost += self.cal_heuristic_func(next_node, goal_node)
                    heapq.heappush(open_set, (next_node.cost, next_node))
...
def visualization(start_x, start_y, goal_x, goal_y, obstacle_x_list, obstacle_y_list, path_x, path_y):
    ...
    plt.plot(path_x, path_y, ".-", color='royalblue', markersize=2)
    plt.show()
...
def main():
    ...
    if show_animation:
        plt.figure(figsize=(12, 12))
        plt.axis("equal")
        plt.plot(obstacle_x_list, obs_list_y, 'sk', markersize=2)
        plt.plot(start_x, start_y, marker='*', color='lime', markersize=8)
    ...
    # # plot searched path
    # if show_animation:
    #     visualization(start_x, start_y, goal_x, goal_y, obstacle_x_list, obstacle_y_list, path_x, path_y)
